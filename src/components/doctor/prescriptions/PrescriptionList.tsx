...
import { useDoctorPrescriptions } from "@/hooks/use-prescription";
...
function PrescriptionListComponent() {
  ...
  const { data, isLoading, error } = useDoctorPrescriptions({
  ...
  const filteredPrescriptions = prescriptions.filter((prescription) => {
    ...
    return patientName.includes(query) || prescription.id.toLowerCase().includes(query);
  });
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-muted-foreground">
                            ...
                            {prescription.appointment && (
                              <div>
                                ...
                                {format(new Date(prescription.appointment.date), "MMM d")}
                              </div>
                            )}
                          </div>
                        </div>
